...
void I2C3_Tx( uint8_t controlByte, uint8_t dataByte )
{
  ...
  I2C3_MDR_R = dataByte ;     // Data to GDDRAM
...
void oled_init( void )
{
  ...
  I2C3_Tx(OLED_COMMAND, 0x77);                     // Set Contrast Value
...
{
  /*
  Function that clears the complete screen :: Under development
  */
  int i ;
  // Page 0 :
  I2C3_Tx(OLED_COMMAND, 0xB0);      // Set Page Start Address
...
void oledPrintStr(char charArray[])
{
  ...
  */
  ...
  int blank_col_fills = 128 -  (6 * inputStrLen) ;
...
void num2str(float num, char *str, int precision) {
    ...
    if (precision > 0) {
        *str++ = '.';  // Add the decimal point
        ...
        int fracPart = (int)num;  // Get the integer part of the fractional part
